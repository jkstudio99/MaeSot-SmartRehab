...
async function init() {
    try {
        ...
        const modelURL = "models/pullback/model.json";
        const metadataURL = "models/pullback/metadata.json";
...
async function predict() {
    ...
    if (highestProbability > 0.9 && mostLikelyClass === "PullBack") {
        if (!timerStarted) {
            startTimer();
            timerStarted = true;
        }
...
function finishExercise() {
    ...
    alert(`การทดสอบเสร็จสิ้น\nคุณทำได้: ${counter} ครั้ง\nเยี่ยมมาก! การดึงแขนไปด้านหลังช่วยเพิ่มความยืดหยุ่นของกล้ามเนื้อบริเวณหัวไหล่และหลัง`);
}
