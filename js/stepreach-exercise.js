...
async function init() {
    try {
        ...
        const modelURL = "models/stepreach/model.json";
        const metadataURL = "models/stepreach/metadata.json";
...
async function predict() {
    ...
    if (highestProbability > 0.9 && mostLikelyClass === "StepReach") {
        if (!timerStarted) {
            startTimer();
            timerStarted = true;
        }
...
function finishExercise() {
    ...
    alert(`การทดสอบเสร็จสิ้น\nคุณทำได้: ${counter} ครั้ง\nเยี่ยมมาก! การก้าวพร้อมยืดแขนช่วยเพิ่มความแข็งแรงของกล้ามเนื้อแขนและขา`);
}
