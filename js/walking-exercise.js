...
async function init() {
    try {
        ...
        const modelURL = "models/walking/model.json";
        const metadataURL = "models/walking/metadata.json";
...
async function predict() {
    ...
    if (highestProbability > 0.9 && mostLikelyClass === "Walking") {
        if (!timerStarted) {
            startTimer();
            timerStarted = true;
        }
...
function finishExercise() {
    ...
    alert(`การทดสอบเสร็จสิ้น\nคุณทำได้: ${counter} ครั้ง\nเยี่ยมมาก! การเดินอยู่กับที่ช่วยกระตุ้นการไหลเวียนเลือดและเผาผลาญพลังงานได้ดี`);
}
